        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-16">
            ...
            <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-border/40 dark:divide-white/10 bg-white/80 dark:bg-[#181C23] rounded-2xl shadow-sm overflow-hidden">
                {features.map((feature, idx) => (
                    ...
                    >
                        <div className="h-8 sm:h-9 mb-3 flex items-center justify-center">
                            {feature.icon}
                        </div>
                        <h3 className="text-md sm:text-lg md:text-xl font-semibold mb-2 text-foreground h-auto md:h-14 flex items-center justify-center">{feature.title}</h3>
                        <p className="text-xs sm:text-sm md:text-base text-muted-foreground max-w-xs mx-auto">{feature.description}</p>
                    </div>
                ))}
            </div>
        </div>
